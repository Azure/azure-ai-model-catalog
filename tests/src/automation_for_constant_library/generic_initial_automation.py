...
if __name__ == "__main__":
    ...
    if (
        test_model_name is None
        or test_sku_type is None
        or test_queue is None
        or test_set is None
        or test_trigger_next_model is None
        or test_keep_looping is None
    ):
        logger.error(
            "::error:: One or more of the environment variables test_model_name, test_sku_type, test_queue, test_set, test_trigger_next_model, test_keep_looping are not set"
        )
        exit(1)
    ...
    # logger.info (f"test_model_name: {model_name}")
    ...
    endpoint_name = queue.compute.lower()
    endpoint = create_endpoint(
        workspace_ml_client=workspace_ml_client, endpoint_name=endpoint_name
    )
    ...
    try:
        ...
        InferenceAndDeployment.model_infernce_and_deployment(
            instance_type=queue.instance_type, endpoint=endpoint
        )
    except Exception as ex:
