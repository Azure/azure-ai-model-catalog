...
class ModelInferenceAndDeployemnt:
    ...
    def get_latest_model_version(self, workspace_ml_client, model_name):
        ...
        if len(version_list) == 0:
            ...
            foundation_model = workspace_ml_client.models.get(model_name, model_version)
            logger.info(
        ...
        try:
            ...
            if task == "fill-mask":
                ...
                for index in range(len(scoring_input.input_data)):
                    scoring_input.input_data[index] = (
                        scoring_input.input_data[index]
                        .replace("<mask>", pipeline_tokenizer.mask_token)
                        .replace("[MASK]", pipeline_tokenizer.mask_token)
                    )
    ...
    def cloud_inference(self, scoring_file, scoring_input, online_endpoint_name, deployment_name, task, latest_model, endpoint):
        try:
            ...
            try:
                ...
                logger.warning(f"::warning:: This is failed due to this :\n {ex}")
                message = str(ex)
    ...
    def create_model_package(self, latest_model, endpoint):
        ...
        try:
            ...
            model_package = self.workspace_ml_client.models.package(
                latest_model.name, latest_model.version, package_config
            )
        except Exception as e:
            _, _, exc_tb = sys.exc_info()
    ...
    def get_model_name(self, latest_model_name):
        ...
        regx_for_expression = re.compile("|".join(map(re.escape, expression_to_ignore)))
        ...
        reserve_keywords_check = re.findall(regx_for_reserve_keyword, latest_model_name)
        if reserve_keywords_check:
            # Replace the resenve keyword with nothing with hyphen
            latest_model_name = regx_for_reserve_keyword.sub("", latest_model_name)
            latest_model_name = latest_model_name.lstrip("-")

        return latest_model_name

    def create_online_deployment(
        self, latest_model, online_endpoint_name, model_package, instance_type
    ):
        ...
        # else:
        #     latest_model_name = latest_model.name
        latest_model_name = self.get_model_name(latest_model_name=latest_model.name)
        ...
        try:
            ...
            self.prase_logs(str(e))
            self.get_online_endpoint_logs(deployment_name, online_endpoint_name)
    ...
    def delete_online_deployment(self, endpoint, online_endpoint_name, deployment_name):
        try:
            ...
            self.workspace_ml_client.online_deployments.begin_delete(
                name=deployment_name, endpoint_name=online_endpoint_name
            ).wait()
        except Exception as e:
            _, _, exc_tb = sys.exc_info()
            logger.error(
    ...
    def model_infernce_and_deployment(self, instance_type, endpoint):
        expression_to_ignore = [
            "/", "\\", "|", "@", "#", ".", "$", "%", "^", "&", "*", "<", ">", "?", "!", "~", "_",
        ]
        # Create the regular expression to ignore
